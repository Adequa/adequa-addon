  ...
  <div>
    <div class="summary">
      ...
      <div v-else class="toggle" :class="{ active }">
        ...
        <toggle-button :value="active" :color="{ checked: 'var(--green)', unchecked: 'var(--red)' }" @change="onClick" :sync="true"></toggle-button>
      </div>
    </div>
...
export default {
  ...
  created() {
    this.active = this.$store.state.purposeList[this.purpose.id].allowed;

    this.$store.subscribe((mutation, state) => {
      if (mutation.type === types.SET_PURPOSE) {
        this.active = state.purposeList[this.purpose.id].allowed;
      }
    });
  },
  computed: {
  ...
  methods: {
    onClick(e) {
      this.$store.commit(types.SET_PURPOSE, { id: this.purpose.id, allowed: e.value });
    },
    expand(e) {
