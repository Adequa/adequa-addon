  ...
  <div class="interest">
    <div class="interest-wrapper">
      <div>
        ...
        <p @click="toggleExpand">{{ interest.category }}</p>
      </div>
      <div class="actions">
        ...
        <font-awesome-icon icon="times" class="remove" @click="remove" v-show="interest.status !== 'deleted'"></font-awesome-icon>
      </div>
    </div>
...
export default {
  ...
  methods: {
    poll() {
      browser.runtime.sendMessage({ what: 'pollCatalogs', interest: this.interest });
      window.close();
    },
    check() {
    ...
    remove() {
      this.$emit('interest-change', Object.assign(this.interest, { status: 'deleted' }));
    },
    toggleExpand(e) {
