...
// First character of match must be within the hostname part of the url.
    ...
    var filterFromCompiledData = function(args, g) {
        if(g){
            args.push(g)
        }
...
//var filterClassHistogram = new Map();
    ...
    FilterPlain.prototype.match = function(url, tokenBeg) {
        var match = url.startsWith(this.s, tokenBeg - this.tokenBeg);
        if(match){
            if (this.g !== undefined)
                return this.g;
        }
    ...
    FilterPlain.load = function(args) {
        return new FilterPlain(args[1], args[2], args[args.length - 1]);
    };
    ...
    FilterPlainPrefix0.prototype.match = function(url, tokenBeg) {
        var match = url.startsWith(this.s, tokenBeg);
        if(match){
            if (this.g !== undefined)
                return this.g;
        }
    ...
    FilterPlainPrefix0.load = function(args) {
        return new FilterPlainPrefix0(args[1], args[args.length - 1]);
    };
    ...
    FilterPlainPrefix1.prototype.match = function(url, tokenBeg) {
        var match = url.startsWith(this.s, tokenBeg - 1);
        if(match){
            if (this.g !== undefined)
                return this.g;
        }
    ...
    FilterPlainPrefix1.load = function(args) {
        return new FilterPlainPrefix1(args[1], args[args.length - 1]);
    };
    ...
    FilterPlainHostname.prototype.match = function() {
        ...
        var match = offset === 0 || haystack.charCodeAt(offset - 1) === 0x2E /* '.' */;
        if(match){
            if (this.g !== undefined)
                return this.g;
        }
    ...
    FilterPlainHostname.load = function(args) {
        return new FilterPlainHostname(args[1], args[args.length - 1]);
    };
    ...
    FilterPlainLeftAnchored.prototype.match = function(url) {
        var match = url.startsWith(this.s);
        if(match){
            if (this.g !== undefined)
                return this.g;
        }
    ...
    FilterPlainLeftAnchored.load = function(args) {
        return new FilterPlainLeftAnchored(args[1], args[args.length - 1]);
    };
    ...
    FilterPlainRightAnchored.load = function(args) {
        return new FilterPlainRightAnchored(args[1], args[args.length - 1]);
    };
    ...
    FilterExactMatch.load = function(args) {
        return new FilterExactMatch(args[1], args[args.length - 1]);
    };
    ...
    FilterGeneric.load = function(args) {
        return new FilterGeneric(args[1], args[2], args[args.length - 1]);
    };
    ...
    FilterGenericHnAnchored.load = function(args) {
        return new FilterGenericHnAnchored(args[1], args[args.length - 1]);
    };
    ...
    FilterRegex.load = function(args) {
        return new FilterRegex(args[1], args[args.length - 1]);
    };
...
// Filtering according to the origin.
    ...
    FilterOrigin.prototype.match = function(url, tokenBeg) {
        var match = this.wrapped.match(url, tokenBeg);
        return this.matchOrigin() ? match : false;
    };
...
//
    var FilterHostnameDict = function(g) {
    ...
    FilterHostnameDict.prototype.compile = function() {
        return [ this.fid, µb.arrayFrom(this.dict), this.g ];
    };

    FilterHostnameDict.load = function(args) {
        var f = new FilterHostnameDict(args[args.length - 1]);
...
// key=  10000 rss          count= 51
    ...
    FilterPair.prototype.match = function(url, tokenBeg) {
        var f1match = this.f1.match(url, tokenBeg);
        var f2match = this.f2.match(url, tokenBeg);
        if ( f1match ) {
            this.f = this.f1;
            return f1match;
        }
...
// Promote hit filters so they can be found faster next time.
    ...
    FilterBucket.prototype.match = function(url, tokenBeg) {
        ...
        for ( var i = 0, n = filters.length; i < n; i++ ) {
            var match = filters[i].match(url, tokenBeg);
            if ( match ) {
                this.f = filters[i];
...
// Using fast/compact dictionary when filter is a pure hostname.
    ...
    FilterContainer.prototype.fromCompiledContent = function(reader) {
        ...
        while ( reader.next() === true ) {
            ...
            //     fdata[Object.keys(fdata).length] = args[args.length - 1];
            // else
            //     fdata = [args[2], args[args.length - 1]];
            ...
            if ( entry === undefined ) {
                bucket.set(tokenHash, filterFromCompiledData(fdata, args[3]));
                continue;
            }
            ...
            if ( entry.fid === filterPairId ) {
                bucket.set(
                    tokenHash,
                    entry.upgrade(filterFromCompiledData(fdata, args[3]))
                );
                continue;
            }
            bucket.set(
                tokenHash,
                new FilterPair(entry, filterFromCompiledData(fdata, args[3]))
            );
        }
    };
...
// url: string
    ...
    FilterContainer.prototype.matchTokens = function(bucket, url) {
        ...
        for (;;) {
            ...
            if ( f !== undefined ) {
                var match = f.match(url, tokenOffset);
                if(match) {
...
//   User may want to override `generichide` exception filters.
    ...
    FilterContainer.prototype.matchStringGenericHide = function(context, requestURL) {
        ...
        if ( bucket ) {
            ...
            var match = this.matchTokens(bucket, url);
            if(match)
                return match;
        }
